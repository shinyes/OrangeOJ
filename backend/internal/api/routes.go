...
func NewApp(db *sql.DB, jwtSecret string, cookieSecure bool, corsOrigins string) *fiber.App {
	...
	spaceAdmin.Put("", api.handleUpdateSpace)
	spaceAdmin.Post("/members", api.handleAddSpaceMember)
	spaceAdmin.Put("/members/:userId", api.handleUpdateSpaceMember)
	spaceAdmin.Post("/members/:userId/reset-password", api.handleSpaceMemberResetPassword)
	spaceAdmin.Post("/problems", api.handleCreateSpaceProblem)
	spaceAdmin.Put("/problems/:problemId", api.handleUpdateSpaceProblem)
	...
	spaceAdmin.Get("/root-problems", api.handleListSpaceRootProblems)
