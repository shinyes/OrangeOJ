...
func NewApp(db *sql.DB, jwtSecret string, cookieSecure bool, corsOrigins string) *fiber.App {
	...
	spaceRead.Get("/homeworks/:homeworkId/draft", api.handleGetHomeworkDraft)
	spaceRead.Put("/homeworks/:homeworkId/draft", api.handleSaveHomeworkDraft)
	spaceRead.Delete("/homeworks/:homeworkId/draft", api.handleDeleteHomeworkDraft)
	...
	protected.Delete("/spaces/:spaceId/homeworks/:homeworkId/targets/:userId", auth.RequireSpaceAdmin(db), api.handleDeleteHomeworkTarget)
