                  ...
                  {form.singleChoice.options.map((option, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 flex items-center gap-1">
                        <Textarea placeholder={`选项 ${OPTION_LABELS[index]}`} value={option} onChange={(e) => updateOption(index, e.target.value)} className="flex-1 min-h-0" rows={Math.max(1, option.split('\n').length)} />
